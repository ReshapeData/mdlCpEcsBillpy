            ...
            if check_order_exists(api_sdk,i[0]['FGODOWNNO'])!=True:
                ...
                model={
                        "Model": {
                            ...
                            "FPurGroupId": {
                                "FNumber": code_conversion(app2,"rds_vw_purchasegroup","purchaseNumber",purchase)
                            },
                            "FPurchaserId": {
                                "FNumber": purchase
                            },
...
def Order_view(app2,value,materialID,FIsFree):
    '''
    采购订单单据查询
    :param value: 订单编码
    :return:
    ...
    sql = f"""
    select a.FID,b.FENTRYID,FGIVEAWAY from t_PUR_POOrder a
    inner join t_PUR_POOrderEntry b
    on a.FID=b.FID
    inner join rds_vw_material c
    on c.FMATERIALID=b.FMATERIALID
    where a.FBILLNO='{value}' and b.FMATERIALID='{materialID}' and b.FGIVEAWAY='{FIsFree}'
    """
...
def checkDataExist(app2, FGODOWNNO):
    '''
    通过FSEQ字段判断数据是否在表中存在
    :param app2:
    :param FSEQ:
    :return:
    '''
    sql = f"select FGODOWNNO from RDS_ECS_SRC_pur_storageacct where FGODOWNNO='{FGODOWNNO}'"
...
def insert_procurement_storage(app2, app3, df,ordernum):
    '''
    采购入库
    :param app2:
    :param data:
    ...
    for i in ordernum:
        ...
        data=df[df['FGODOWNNO'] == i]

        if checkDataExist(app3, data.iloc[0]['FGODOWNNO']):
...
def getOdernumber(app2,FNumber):
    '''
    查看上游单据是否存在
    :param app2:
    :param FNumber:
    ...
    sql=f"select FBILLNO from t_PUR_POOrder where FBILLNO='{FNumber}'"
...
def json_model(app2,model_data,api_sdk):
    ...
    result = Order_view(app2, str(model_data['FBILLNO']), materialId,int(model_data['FIsFree']))
    ...
    if result!=[] and materialId!="":
            ...
            model={
                    ...
                    "FStockID": {
                        "FNumber": "SK01" if model_data['FSTOCKID']=='苏州总仓' or model_data['FSTOCKID']=='样品仓' else code_conversion(app2,"rds_vw_warehouse","FNAME",model_data['FSTOCKID'])
                    },
                    "FStockStatusId": {
                        "FNumber": "KCZT02_SYS"
                    },
                    ...
                    "FDetailEntity_Link": [
                      {
                        ...
                        "FDetailEntity_Link_FSBillId": result["FID"],
                        "FDetailEntity_Link_FSId": result["FENTRYID"],
...
def writeSRC(startDate, endDate,app2, app3):
    ...
    if page:
        ...
        for i in range(1, int(page) + 1):
            ...
            insert_procurement_storage(app2,app3, df,odernum)

    pass


def receiptNotice(startDate,endDate,app2,app3,option):
...
def receiptNotice_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
