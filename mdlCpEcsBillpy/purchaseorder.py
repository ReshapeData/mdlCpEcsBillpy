            ...
            if check_order_exists(api_sdk,str(i[0]['FPURORDERNO']))!=True:
                    ...
                    model={
                            "Model": {
                                ...
                                "FPurchaserGroupId": {
                                    "FNumber": code_conversion(app2,"rds_vw_purchasegroup","FNAME",str(i[0]['FPURCHASERID']))
                                },
                                "FPurchaserId": {
                                ...
                                "FPOOrderFinance": {
                                    "FSettleModeId": {
                                        "FNumber": "JSFS04_SYS" if str(i[0]['FSETTLETYPEID'])=="" else code_conversion(app2,"rds_vw_settlement","FNAME",str(i[0]['FSETTLETYPEID']))
                                    },
                                    "FPayConditionId": {
                                        "FNumber": code_conversion(app2,"rds_vw_payment","FNAME",str(i[0]['FRECCONDITIONID']))
                                    },
                                    "FSettleCurrId": {
                                        "FNumber": "PRE001"
                                    },
...
def checkDataExist(app2, FPURORDERNO):
    '''
    通过FSEQ字段判断数据是否在表中存在
    :param app2:
    :param FSEQ:
    :return:
    '''
    sql = f"select FPURORDERNO from RDS_ECS_SRC_pur_poorder where FPURORDERNO='{FPURORDERNO}'"
...
def insert_procurement_order(app2,app3,odernum,api_sdk,option,df):
    '''
    采购订单
    :param app2:
    :param data:
...
def judgementData(app2, app3, data):
    ...
    if code_conversion_org(app2, "rds_vw_supplier", "FNAME", data.iloc[0]['FSUPPLIERNAME'],"104") != "":
...
def inert_data(app3,data):
    ...
    for i in data.index:
        ...
        sql = f"""insert into RDS_ECS_SRC_pur_poorder(FPURORDERNO,FBILLTYPENAME,FPURCHASEDATE,FCUSTOMERNUMBER,FSUPPLIERNAME,FPOORDERSEQ,FPRDNUMBER,FPRDNAME,FQTY,FPRICE,FAMOUNT,FTAXRATE,FTAXAMOUNT,FTAXPRICE,FORAMOUNTFALL,FPURCHASEDEPTID,FPURCHASEGROUPID,FPURCHASERID,FUploadDate,FIsDo,FIsFree,FUpDateTime,FOrderId,FDESCRIPTION,FStatus,FRECCONDITIONID,FSETTLETYPEID) values('{data.loc[i]['FPURORDERNO']}','{data.loc[i]['FBILLTYPENAME']}','{data.loc[i]['FPURCHASEDATE']}','{data.loc[i]['FCUSTOMERNUMBER']}','{data.loc[i]['FSUPPLIERNAME'].replace("'","''")}','{data.loc[i]['FPOORDERSEQ']}','{data.loc[i]['FPRDNUMBER']}','{data.loc[i]['FPRDNAME'].replace("'","''")}','{data.loc[i]['FQTY']}','{data.loc[i]['FPRICE']}','{data.loc[i]['FAMOUNT']}','{data.loc[i]['FTAXRATE']}','{data.loc[i]['FTAXAMOUNT']}','{data.loc[i]['FTAXPRICE']}','{data.loc[i]['FAMOUNT']}','{data.loc[i]['FPURCHASEDEPTID']}','{data.loc[i]['FPURCHASEGROUPID']}','{data.loc[i]['FPURCHASERID']}',getdate(),0,0,'{data.loc[i]['UPDATETIME']}','{data.loc[i]['FOrderId']}','{data.loc[i]['FDESCRIPTION']}','{data.loc[i]['FStatus']}','{data.loc[i]['FRECCONDITIONID']}','{data.loc[i]['FSETTLETYPEID']}')"""
...
def writeSRC(startDate, endDate,app2, app3,api_sdk,option):
    ...
    if page:
        ...
        for i in range(1, int(page) + 1):
            ...
            insert_procurement_order(app2, app3, odernum, api_sdk, option,df)



def purchaseOrder(startDate,endDate,app2,app3,option):
    '''
    接口入口
    :return:
    '''
...
def purchaseOrder_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
