        ...
        try:
            ...
            if check_outstock_exists(api_sdk, i[0]['FMRBBILLNO']) != True:
                ...
                if res['Result']['ResponseStatus']['IsSuccess']:
                    ...
                    insertLog(app3, "采购退料单", i[0]['FMRBBILLNO'],
...
def delivery_view(api_sdk, value):
    ...
    res = json.loads(api_sdk.ExecuteBillQuery({"FormId": "PUR_MRAPP", "FieldKeys": "FDate,FBillNo,FId,FEntity_FEntryID",
                                               "FilterString": [
                                                   {"Left": "(", "FieldName": "FBillNo", "Compare": "=", "Value": value,
                                                    "Right": ")", "Logic": "AND"}], "TopRowCount": 0}))
...
def json_model(app2, model_data, value):
    try:
        ...
        if model_data['FGOODSID'] == '1' or str(
                code_conversion(app2, "rds_vw_material", "F_SZSP_SKUNUMBER", model_data['FGOODSID'])):

            model = {
                "FRowType": "Standard" if model_data['FGOODSID'] != '1' else "Service",
                "FMATERIALID": {
                    "FNumber": "7.1.000001" if model_data['FGOODSID'] == '1' else str(
                        code_conversion(app2, "rds_vw_material", "F_SZSP_SKUNUMBER", model_data['FGOODSID']))
                },
                ...
                "FSTOCKID": {
                    "FNumber": "SK01" if model_data['FSTOCKID'] == "苏州总仓" or model_data['FSTOCKID'] == "" or model_data[
                        'FSTOCKID'] == "样品仓" else code_conversion(app2, "rds_vw_warehouse", "FNAME",
                                                                  model_data['FSTOCKID'])
                },
                "FStockStatusId": {
                    "FNumber": "KCZT01_SYS"
                ...
                "FALLAMOUNT": str(model_data['FRETAMOUNT']),
                "FALLAMOUNT_LC": str(model_data['FRETAMOUNT']),
                "FLot": {
                    "FNumber": str(model_data['FLOT']) if isbatch(app2, model_data['FGOODSID']) == '1' else ""
                },
                "FProduceDate": str(model_data['MANUFACTUREDATE']) if iskfperiod(app2,
                                                                                 model_data['FGOODSID']) == '1' else "",
...
def returnPurchase(startDate, endDate, app2, app3, option):
...
def returnPurchase_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
