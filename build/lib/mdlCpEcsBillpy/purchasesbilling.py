...
def ERP_Save(api_sdk,data,option,app2,app3):
    ...
    for i in data:
        ...
        try:
            ...
            if check_order_exists(api_sdk,str(i[0]['FBILLNO']))!=True:
                    ...
                    model={
                        "Model": {
                            ...
                            "F_SZSP_FPHM": FNo,
                            "F_SZSP_FPRQJSY": str(i[0]['FINVOICEDATE']),
                            "FsubHeadSuppiler": {
                                "FORDERID": {
                                    "FNumber": code_conversion_org(app2,"rds_vw_supplier","FNAME",i[0]['FSUPPLIERNAME'],"104","FNUMBER")
                                },
                    ...
                    if save_result['Result']['ResponseStatus']['IsSuccess']:
                        ...
                        insertLog(app3, "应付单", str(i[0]['FBILLNO']), "数据同步成功", "1")
                        ...
                        insertLog(app3, "应付单", str(i[0]['FBILLNO']), save_result['Result']['ResponseStatus']['Errors'][0]['Message'],"2")
                ...
                insertLog(app3, "应付单", str(i[0]['FBILLNO']),
                          "数据同步成功", "1")
            ...
            insertLog(app3, "应付单", str(i[0]['FBILLNO']), "数据异常","2")
...
def json_model(app2,model_data,api_sdk):
        ...
        if str(model_data['FGODOWNNO']).find("SR")==-1:

            result=checkCode(app2,str(model_data['FGODOWNNO']),str(model_data['FQTY']),str(model_data['FLot']),materialSKU)

        else:

            result = checkCode2(app2, str(model_data['FGODOWNNO']), str(model_data['FQTY']), str(model_data['FLot']),
                               materialSKU)
        ...
        if result != [] and materialId != "":
            ...
            model={
                    ...
                    "FTAXAMOUNTFOR_D": str(model_data['FTAXAMOUNT']),
                    "FALLAMOUNTFOR_D": str(model_data['FAMOUNTALL']),
                    "FINCLUDECOST": False,
                    "FISOUTSTOCK": False,
                    "FLot": {
                    ...
                    "FTAILDIFFFLAG": False,
                    "FROOTSOURCETYPE": "STK_InStock",
                    "FEntityDetail_Link": [{
                        "FEntityDetail_Link_FRuleId": "AP_InStockToPayableMap" if str(model_data['FGODOWNNO']).find("SR")==-1 else "AP_MRBToPayableMap",
                        "FEntityDetail_Link_FSTableName": "T_STK_INSTOCKENTRY" if str(model_data['FGODOWNNO']).find("SR")==-1 else "T_PUR_MRBENTRY",
                        ...
                        "FEntityDetail_Link_FALLAMOUNTFOR_DOld":str(model_data['FQTY']),
                        "FEntityDetail_Link_FALLAMOUNTFOR_D":str(model_data['FQTY'])
                    }]
                }
...
def insert_procurement_contract(app2,app3,data):
    ...
    for i in data.index:
        ...
        if checkDataExist(app3,data.loc[i]['FInvoiceid']):
            ...
            sql = f"""insert into RDS_ECS_SRC_pur_invoice(FPURORDERNO,FGODOWNNO,FBILLTYPEINAME,FINVOICEDATE,FINVOICETYPE,
                FINVOICENO,FDATE,FCUSTOMERNUMBER,FSUPPLIERNAME,FPOORDERSEQ,FPRDNUMBER,
                FPRDNAME,FQTY,FUNITPRICE,FSUMVALUE,FTAXRATE,FTAXAMOUNT,
                FTAXPRICE,FAMOUNTALL,FPURCHASEDEPTNAME,FPURCHASEGROUPNAME,
                FPURCHASERINAME,FDESCRIPTION,FUPLOADDATE,FISDO,FInvoiceid,
                FLot,FBILLNO) values('{data.loc[i]['FPURORDERNO']}','{data.loc[i]['FGODOWNNO']}',
                '{data.loc[i]['FBILLTYPEID']}','{data.loc[i]['FINVOICEDATE']}',
                '{data.loc[i]['FINVOICETYPE']}','{data.loc[i]['FINVOICENO']}',
                '{data.loc[i]['FRECEIVABLETIME']}','{data.loc[i]['FCUSTOMERNUMBER']}',
                '{data.loc[i]['FSUPPLIERNAME'].replace("'", "''")}','{data.loc[i]['FPOORDERSEQ']}',
                '{data.loc[i]['FPRDNUMBER']}','{data.loc[i]['FPRDNAME'].replace("'", "''")}',
                '{data.loc[i]['FQTY']}','{data.loc[i]['FUNITPRICE']}','{data.loc[i]['FSUMVALUE']}',
                '{data.loc[i]['FTAXRATE']}','{data.loc[i]['FTAXAMOUNT']}','{data.loc[i]['FTAXPRICE']}',
                '{data.loc[i]['FAMOUNT']}','{data.loc[i]['FPURCHASEDEPTID']}',
                '{data.loc[i]['FPURCHASEGROUPID']}','{data.loc[i]['FPURCHASERID']}',
                '{data.loc[i]['FDESCRIPTION']}',getdate(),0,'{data.loc[i]['FInvoiceid']}',
                '{data.loc[i]['FLOT']}','{data.loc[i]['FBILLNO']}')"""
            app3.insert(sql)
...
def getCode(app3):
    ...
    sql = "select distinct FBILLNO from RDS_ECS_ODS_pur_invoice where FIsDo=0"
...
def getClassfyData(app3, code):
    ...
    try:
        ...
        sql = f"""select FPURORDERNO,FGODOWNNO,FBILLTYPEINAME,FINVOICEDATE,FINVOICETYPE,
                FINVOICENO,FDATE,FCUSTOMERNUMBER,FSUPPLIERNAME,FPOORDERSEQ,
                FPRDNUMBER,FPRDNAME,FQTY,FUNITPRICE,FSUMVALUE,FTAXRATE,
                FTAXAMOUNT,FTAXPRICE,FAMOUNTALL,FPURCHASEDEPTNAME,FPURCHASEGROUPNAME,
                FPURCHASERINAME,FDESCRIPTION,FUPLOADDATE,FISDO,FInvoiceid,FLot,FBILLNO 
                from RDS_ECS_ODS_pur_invoice where FBILLNO='{number}'"""
...
def changeStatus(app3,fnumber,status):
    ...
    sql=f"update a set a.Fisdo={status} from RDS_ECS_ODS_pur_invoice a where FBILLNO='{fnumber}'"
...
def checkCode2(app2,FNumber,FralQty,FLot,FSKU):
    '''
    查看采购入库单单据内码
    :param app2:
    :param FNumber: 单据编码
    ...
    try:
        ...
        sql=f"""
            select a.FID,b.FENTRYID,b.FLOT,b.FLOT_TEXT,c.FNUMBER from t_PUR_MRB a
            inner join T_PUR_MRBENTRY b
            on a.FID=b.FID
            inner join rds_vw_material c
            on c.FMATERIALID=b.FMATERIALID
            where a.FBILLNO='{FNumber}' and b.FRMREALQTY='{FralQty}' and b.FLOT_TEXT='{FLot}' and c.F_SZSP_SKUNUMBER='{FSKU}'          
            """
...
def writeSRC(startDate, endDate, app2,app3):
    ...
    page = viewPage(url, 1, 1000, "ge", "le", "v_procurement_contract", startDate, endDate, "UPDATETIME")

    if page:

        for i in range(1, int(page) + 1):
            df = ECS_post_info2(url, i, 1000, "ge", "le", "v_procurement_contract", startDate, endDate, "UPDATETIME")
...
def purchasesBilling(startDate, endDate,app2,app3,option):
    ...
    if data:
        ...
        res=ERP_Save(api_sdk=api_sdk, data=res, option=option, app2=app2, app3=app3)
...
def purchasesBilling_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
