...
def insert_procurement_return(app2,app3,data):
    ...
    for i in data.index:
        ...
        if checkDataExist(app3,data.loc[i]['FReturnId']):
            sql = "insert into RDS_ECS_SRC_pur_return(FMRBBILLNO,FPURORDERNO,FPOORDERSEQ,FBILLTYPEID,FCUSTOMERNUMBER,FSUPPLIERFIELD,FSUPPLIERNAME,FSUPPLIERABBR,FSTOCKID,FGOODSTYPEID,FBARCODE,FGOODSID,FPRDNAME,FRETSALEPRICE,FTAXRATE,FLOT,FRETQTY,FRETAMOUNT,FCHECKSTATUS,FUploadDate,FIsDo,FINISHTIME,FDATE,MANUFACTUREDATE,EFFECTDATE,FReturnId) values('" + \
                  str(data.loc[i]['FMRBBILLNO']) + "','" + str(data.loc[i]['FPURORDERNO']) + "','" + str(data.loc[i][
                                                                                                             'FPOORDERSEQ']) + "','" + str(
                data.loc[i]['FBILLTYPEID']) + "','" + str(data.loc[i][
                                                              'FSUPPLIERFIELD']) + "','" + str(
                data.loc[i]['FCUSTOMERNUMBER']) + "','" + str(data.loc[i][
                                                                  'FSUPPLIERNAME'].replace("'", "''")) + "','" + str(
                data.loc[i]['FSUPPLIERABBR']) + "','" + str(data.loc[i][
                                                                'FSTOCKID']) + "','" + str(
                data.loc[i]['FGOODSTYPEID']) + "','" + str(data.loc[i]['FBARCODE']) + "','" + \
                  str(data.loc[i]['FGOODSID']) + "','" + str(data.loc[i]['FPRDNAME'].replace("'", "''")) + "','" + str(
                data.loc[i][
                    'FRETSALEPRICE']) + "','" + str(data.loc[i]['FTAXRATE']) + "','" + str(
                data.loc[i]['FLOT']) + "','" + \
                  str(data.loc[i]['FRETQTY']) + "','" + str(data.loc[i]['FRETAMOUNT']) + "','" + str(data.loc[i][
                                                                                                         'FCHECKSTATUS']) + "',getdate(),0,'" + str(
                data.loc[i]['FINISHTIME']) + "','" + str(data.loc[i][
                                                             'FDATE']) + "','" + str(
                data.loc[i]['MANUFACTUREDATE']) + "','" + str(data.loc[i]['EFFECTDATE']) + "','" + \
                  str(data.loc[i]['FReturnId']) + "')"
...
def returnRequest(startDate,endDate,app2,app3,option):
...
def returnRequest_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
