    ...
    for i in data:
        ...
        try:
            ...
            if check_order_exists(api_sdk, i[0]['FBILLNO']) != True:
                ...
                insertLog(app3, "应收单", str(i[0]['FBILLNO']),
                          "数据同步成功", "1")
    ...
    try:
        ...
        if result != [] and materialId != "":
            ...
            model = {
                ...
                "FTAILDIFFFLAG": False,
                "FROOTSOURCETYPE": "SAL_OUTSTOCK",
                "FEntityDetail_Link": [{
                    "FEntityDetail_Link_FRuleId": "AR_OutStockToReceivableMap" if int(model_data['FQUANTITY'])>0 else "AR_ReturnToReceivableMap",
                    "FEntityDetail_Link_FSTableName": "T_SAL_OUTSTOCKENTRY" if int(model_data['FQUANTITY'])>0 else "T_SAL_RETURNSTOCKENTRY",
                    "FEntityDetail_Link_FSBillId ": result[index]['Fid'] if int(model_data['FQUANTITY'])>0 else result[index]['FID'],
...
def insert_sales_invoice(app2,app3, data):
    ...
    for i in data.index:
        ...
        if checkDataExist(app3, data.iloc[i]['FInvoiceid']) and data.iloc[i]['FQUANTITY'] != '':
            ...
            sql = f"""insert into RDS_ECS_SRC_sal_billreceivable(FInterID,FCUSTNUMBER,FOUTSTOCKBILLNO,FSALEORDERENTRYSEQ,FBILLTYPEID,FCUSTOMNAME,FBILLNO,FPrdNumber,FPrdName,FQUANTITY,FUNITPRICE,FSUMVALUE,FTAXRATE,FTRADENO,FNOTETYPE,FISPACKINGBILLNO,FBILLCODE,FINVOICENO,FINVOICEDATE,UPDATETIME,Fisdo,FCurrencyName,FInvoiceid,FLot) values({int(getFinterId(app3, 'RDS_ECS_SRC_sal_billreceivable')) + 1},'{data.loc[i]['FCUSTNUMBER']}','{data.loc[i]['FOUTSTOCKBILLNO']}','{data.loc[i]['FSALEORDERENTRYSEQ']}','{data.loc[i]['FBILLTYPEID']}','{data.loc[i]['FCUSTOMNAME'].replace("'", "''")}','{data.loc[i]['FBILLNO']}','{data.loc[i]['FPrdNumber']}','{data.loc[i]['FPrdName'].replace("'", "''")}','{int(data.loc[i]['FQUANTITY'])}','{data.loc[i]['FUNITPRICE']}','{data.loc[i]['FSUMVALUE']}','{data.loc[i]['FTAXRATE']}','{data.loc[i]['FTRADENO']}','{data.loc[i]['FNOTETYPE']}','{data.loc[i]['FISPACKINGBILLNO']}','{data.loc[i]['FBILLCODE']}','{data.loc[i]['FINVOICENO']}','{data.loc[i]['FINVOICEDATE']}',getdate(),0,'{data.loc[i]['FCURRENCYID']}','{data.loc[i]['FInvoiceid']}','{data.loc[i]['FLOT']}')"""
...
def inert_data(app3,data):
    ...
    for i in range(0,len(data)):
        ...
        sql = f"""insert into RDS_ECS_SRC_sal_billreceivable(FInterID,FCUSTNUMBER,FOUTSTOCKBILLNO,FSALEORDERENTRYSEQ,FBILLTYPEID,FCUSTOMNAME,FBILLNO,FPrdNumber,FPrdName,FQUANTITY,FUNITPRICE,FSUMVALUE,FTAXRATE,FTRADENO,FNOTETYPE,FISPACKINGBILLNO,FBILLCODE,FINVOICENO,FINVOICEDATE,UPDATETIME,Fisdo,FCurrencyName,FInvoiceid,FLot) values({int(getFinterId(app3, 'RDS_ECS_SRC_sal_billreceivable')) + 1},'{data.iloc[i]['FCUSTNUMBER']}','{data.iloc[i]['FOUTSTOCKBILLNO']}','{data.iloc[i]['FSALEORDERENTRYSEQ']}','{data.iloc[i]['FBILLTYPEID']}','{data.iloc[i]['FCUSTOMNAME'].replace("'","''")}','{data.iloc[i]['FBILLNO']}','{data.iloc[i]['FPrdNumber']}','{data.iloc[i]['FPrdName'].replace("'","''")}','{int(data.iloc[i]['FQUANTITY'])}','{data.iloc[i]['FUNITPRICE']}','{data.iloc[i]['FSUMVALUE']}','{data.iloc[i]['FTAXRATE']}','{data.iloc[i]['FTRADENO']}','{data.iloc[i]['FNOTETYPE']}','{data.iloc[i]['FISPACKINGBILLNO']}','{data.iloc[i]['FBILLCODE']}','{data.iloc[i]['FINVOICENO']}','{data.iloc[i]['FINVOICEDATE']}',getdate(),0,'{data.iloc[i]['FCURRENCYID']}','{data.iloc[i]['FInvoiceid']}','{data.iloc[i]['FLOT']}')"""
...
def data_splicing(app2, api_sdk, data):
    ...
    for i in data:
        ...
        # materialSKU = "1" if str(i['FPrdNumber']) == '1' else str(i['FPrdNumber'])
        materialSKU = str(i['FPrdNumber'])
        ...
        if int(i['FQUANTITY'])>0:
            ...
            result = checkFlot(app2, str(i['FOUTSTOCKBILLNO']), Flot,
...
def salesBilling(startDate, endDate,app2,app3,option):
...
def salesBilling_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
