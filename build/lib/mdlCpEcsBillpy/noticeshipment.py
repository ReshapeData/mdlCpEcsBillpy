...
def insert_sales_delivery(app2,app3,data):
    ...
    for i in data.index:
        ...
        if data.loc[i]['FNBASEUNITQTY']!=0 and checkDataExist(app3,data.loc[i]['FOUTID']):
            ...
            sql = f"""insert into RDS_ECS_SRC_sal_delivery(FInterID,FDELIVERYNO,FTRADENO,FBILLTYPE,FDELIVERYSTATUS,FDELIVERDATE,FSTOCK,FCUSTNUMBER,FCUSTOMNAME,FORDERTYPE,FPRDNUMBER,FPRDNAME,FPRICE,FNBASEUNITQTY,FLOT,FSUMSUPPLIERLOT,FPRODUCEDATE,FEFFECTIVEDATE,FMEASUREUNIT,DELIVERYAMOUNT,FTAXRATE,FSALER,FAUXSALER,Fisdo,FArStatus,FIsfree,UPDATETIME,FOUTID,FCurrencyName) values({getFinterId(app3, 'RDS_ECS_SRC_sal_delivery') + 1},'{data.loc[i]['FDELIVERYNO']}','{data.loc[i]['FTRADENO']}','{data.loc[i]['FBILLTYPEID']}','{data.loc[i]['FDELIVERYSTATUS']}','{data.loc[i]['FDELIVERDATE']}','{data.loc[i]['FSTOCKID']}','{data.loc[i]['FCUSTNUMBER']}','{data.loc[i]['FCUSTOMNAME'].replace("'", "''")}','{data.loc[i]['FORDERTYPE']}','{data.loc[i]['FPRDNUMBER']}','{data.loc[i]['FPRDNAME'].replace("'", "''")}','{data.loc[i]['FPRICE']}','{data.loc[i]['FNBASEUNITQTY']}','{data.loc[i]['FLOT']}','{data.loc[i]['FSUMSUPPLIERLOT']}','{data.loc[i]['FPRODUCEDATE']}','{data.loc[i]['FEFFECTIVEDATE']}','{data.loc[i]['FMEASUREUNITID']}','{data.loc[i]['DELIVERYAMOUNT']}','{data.loc[i]['FTAXRATE']}','{data.loc[i]['FSALERID']}','{data.loc[i]['FAUXSALERID']}',0,0,0,'{data.loc[i]['UPDATETIME']}','{data.loc[i]['FOUTID']}','{data.loc[i]['FCURRENCYID']}')"""
...
def getInternalcode(app2,FBIllNO,FMaterial,FIsFree):
    '''
    查看上游单据内码
    :param app2:
    :param FBIllNO: 单据编号
    :param FMaterial:  物料内码
    :param FIsFree:  是否赠品
    :return:
    '''
    sql = f"""
    select a.FBILLNO,a.FID,b.FENTRYID,d.FISFREE from T_SAL_ORDER a
    inner join T_SAL_ORDERENTRY b
    on a.FID=b.FID
    inner join T_SAL_ORDERENTRY_F d
    on a.FID=b.FID and b.FENTRYID=d.FENTRYID
    where a.FBILLNO='{FBIllNO}' and b.FMATERIALID='{FMaterial}' and d.FISFREE='{FIsFree}'
    """
...
def json_model(app2,model_data,api_sdk):
    ...
    result=getInternalcode(app2,str(model_data['FTRADENO']),materialId,int(model_data['FIsfree']))
    ...
    if result!=[] and materialId!="":
        ...
        model={
                ...
                "FEntity_Link": [{
                    ...
                    "FEntity_Link_FSBillId ": result[0]['FID'],
                    "FEntity_Link_FSId ": result[0]['FENTRYID'],
...
def noticeShipment(startDate,endDate,app2,app3,option):
...
def noticeShipment_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
