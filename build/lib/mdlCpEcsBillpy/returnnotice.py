    ...
    for i in data:
        ...
        try:
            ...
            if check_deliveryExist(api_sdk,i[0]['FMRBBILLNO'])!=True:
                ...
                insertLog(app3, "退货通知单", i[0]['FMRBBILLNO'], "数据同步成功",
                          "1")
...
def insert_sales_return(app2,app3,data):
    ...
    for i in data.index:
        ...
        if checkDataExist(app3,data.iloc[i]['FADDID']):
            ...
            sql = f"""insert into RDS_ECS_SRC_sal_returnstock(FMRBBILLNO,FTRADENO,FSALEORDERENTRYSEQ,FBILLTYPE,FRETSALESTATE,FPRDRETURNSTATUS,OPTRPTENTRYDATE,FSTOCK,FCUSTNUMBER,FCUSTOMNAME,FCUSTCODE,FPrdNumber,FPrdName,FRETSALEPRICE,FRETURNQTY,FREQUESTTIME,FBUSINESSDATE,FCOSTPRICE,FMEASUREUNIT,FRETAMOUNT,FTAXRATE,FLOT,FSALER,FAUXSALER,FSUMSUPPLIERLOT,FPRODUCEDATE,FEFFECTIVEDATE,FCHECKSTATUS,UPDATETIME,FDELIVERYNO,FIsDo,FIsFree,FADDID,FCurrencyName) values('{data.loc[i]['FMRBBILLNO']}','{data.loc[i]['FTRADENO']}','{data.loc[i]['FSALEORDERENTRYSEQ']}','{data.loc[i]['FBILLTYPEID']}','{data.loc[i]['FRETSALESTATE']}','{data.loc[i]['FPRDRETURNSTATUS']}','{data.loc[i]['OPTRPTENTRYDATE']}','{data.loc[i]['FSTOCKID']}','{data.loc[i]['FCUSTNUMBER']}','{data.loc[i]['FCUSTOMNAME'].replace("'", "''")}','{data.loc[i]['FCUSTCODE']}','{data.loc[i]['FPRDNUMBER']}','{data.loc[i]['FPRDNAME'].replace("'", "''")}','{data.loc[i]['FRETSALEPRICE']}','{data.loc[i]['FRETURNQTY']}','{data.loc[i]['FREQUESTTIME']}','{data.loc[i]['FBUSINESSDATE']}','{data.loc[i]['FCOSTPRICE']}','{data.loc[i]['FMEASUREUNITID']}','{data.loc[i]['FRETAMOUNT']}','{data.loc[i]['FTAXRATE']}','{data.loc[i]['FLOT']}','{data.loc[i]['FSALERID']}','{data.loc[i]['FAUXSALERID']}','{data.loc[i]['FSUMSUPPLIERLOT']}','{data.loc[i]['FPRODUCEDATE']}','{data.loc[i]['FEFFECTIVEDATE']}','{data.loc[i]['FCHECKSTATUS']}','{data.loc[i]['UPDATETIME']}','{data.loc[i]['FDELIVERYNO']}',0,0,'{data.loc[i]['FADDID']}','{data.loc[i]['FCURRENCYID']}')"""
...
def returnNotice(startDate,endDate,app2,app3,option):
...
def returnNotice_byOrder(app2,app3,option,data):
    '''
    按单据同步
    :param startDate:
    :param endDate:
